...
@Component({
  selector: 'nc-web-public-home',
  templateUrl: './public-home.component.html',
  styleUrl: './public-home.component.scss'
})
export class PublicHomeComponent implements OnInit, OnChanges {
        ...
        if (response.status === 'success') {
          ...
          if (this?.metaData?.['NEWS_CATEGORIES_REGIONAL']?.length > 0)
            for (let index = 0; index < data.length; index++) {
              ...
              let ind = this.metaData?.['NEWS_CATEGORIES_REGIONAL'].findIndex((elem:any) => elem.label === data[index]['category']);
              if (ind > -1) {
